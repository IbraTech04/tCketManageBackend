...
class CryptoServiceTest {
    ...
    void setUp() throws Exception {
        ...
        cryptoService = new CryptoService(testPrivateKey, testPublicKey, new ObjectMapper());
    }
    ...
    void testValidateSignature_DifferentKeyPair_ShouldReturnFalse() throws Exception {
        // Arrange - Create a different key pair
        KeyPairGenerator keyPairGenerator = KeyPairGenerator.getInstance("RSA");
        keyPairGenerator.initialize(2048);
        ...
        // Sign with different key pair
        differentCryptoService.signTicket(ticket);

        // Act - Validate with original key pair
        boolean isValid = cryptoService.validateSignature(ticket);

        // Assert
        assertFalse(isValid, "Signature from different key pair should be invalid");
    }
    ...
    void testToBase64_WithMockedObjectMapper_ShouldHandleException() throws Exception {
        // Arrange
        CryptoService cryptoServiceWithMock = new CryptoService(testPrivateKey, testPublicKey, mockObjectMapper);
        TicketQRData ticket = TicketQRData.builder()
                .ticketID(UUID.randomUUID())
                .build();

        when(mockObjectMapper.writeValueAsString(any())).thenThrow(new RuntimeException("JSON processing error"));
